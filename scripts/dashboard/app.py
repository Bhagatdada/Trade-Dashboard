...
export_file_path = "../../data/cleaned_Export_trade_data.xlsx"  # Path for export data
import_file_path = "../../data/cleaned_Import_trade_data.xlsx"  # Path for import data
...
app.layout = html.Div(
    style={"backgroundColor": "#f7f7f7", "color": "#424242", "padding": "20px"},  # Light background for the app
    children=[
        html.H1("India-Australia Trade Data Visualization", style={"textAlign": "center", "color": "#333333"}),

        # Updated layout for vertical stack (column layout)
        html.Div(
            style={"display": "flex", "flexDirection": "column", "alignItems": "center", "color": "#333333"},
            children=[
                # Import Data Visualization
                html.Div(
                    style={"width": "80%", "marginBottom": "30px"},
                    children=[
                        html.H2("Import Trade Data", style={"textAlign": "center", "color": "#3498db"}),
                        dcc.Dropdown(
                            ...
                            style={"color": "#333333", "backgroundColor": "#ecf0f1"},
                        ),
                        dcc.Dropdown(
                ...
                html.Div(
                    style={"width": "80%"},
                    children=[
                        html.H2("Export Trade Data", style={"textAlign": "center", "color": "#2ecc71"}),
                        dcc.Dropdown(
...
def update_import_graph(selected_commodity, selected_year):
    ...
    # Apply filters based on commodity and year selections
    ...
    # If no filters, show the total import data (default view)
        ...
        return {
            "data": [
                go.Scatter(
                    ...
                    mode="lines+markers",
                    line=dict(color="#f39c12"),  # Soft yellow line
                    marker=dict(size=8),
                )
            ],
            "layout": go.Layout(
                ...
                plot_bgcolor="#ecf0f1",  # Lighter plot background color
                paper_bgcolor="#ffffff",  # Light paper background color
                font=dict(color="#333333"),
            ),
        }

    # Return the figure based on filtered data
    if not filtered_data.empty:
        return {
            ...
            "layout": go.Layout(
                title=f"Import Trade Data for {selected_commodity} ({selected_year})" if selected_year else f"Import Trade Data for {selected_commodity}",
                xaxis={"title": "Year"},
                yaxis={"title": "Values in US$ Million"},
...
def update_export_graph(selected_commodity, selected_year):
    ...
    # If no filters, show the total export data (default view)
    if not selected_commodity and not selected_year:
        return {
            "data": [
                go.Scatter(
                    ...
                    mode="lines+markers",
                    line=dict(color="#1abc9c"),  # Soft teal line
                    marker=dict(size=8),
                )
            ],
            "layout": go.Layout(
    ...
    if not filtered_data.empty:
        return {
            ...
            "layout": go.Layout(
                title=f"Export Trade Data for {selected_commodity} ({selected_year})" if selected_year else f"Export Trade Data for {selected_commodity}",
                xaxis={"title": "Year"},
                yaxis={"title": "Values in US$ Million"},
                yaxis_range=[0, filtered_data["Trade_Value"].max() * 1.1],
                plot_bgcolor="#ecf0f1",  # Lighter plot background color
